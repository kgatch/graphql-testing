...
Feature: GraphQL testing for a travel product from https://traveller-core.dev.pelago.co/graphql?
	...
	Scenario: Query all fields of the travel product
		...
		Then the JSON response should have "data.product.productId" of type string and value "pewuq"
		Then the JSON response should have "data.product.productName" of type string and value "Singapore Pub Crawl"
	...
	Scenario Outline: Query fields of the travel product one by one
		...
		|productId				|data.product.productId					|string			|pewuq						|
		|productName			|data.product.productName				|string			|Singapore Pub Crawl		|
